...
function renderWithCode(text: string) {
  ...
  return parts.map((part, i) =>
    part.startsWith('`') && part.endsWith('`')
      ? <code key={i} className="inline text-xs px-1 py-0.5 rounded" style={{ fontFamily: "'JetBrains Mono', monospace", color: 'var(--text-400)', background: 'rgba(255,255,255,0.08)', wordBreak: 'break-word' }}>{part.slice(1, -1)}</code>
      : part,
  );
}
...
export default function NodeDetailsPanel({ node, onClose, isClosing = false }: Props) {
  ...
  const color = 'var(--node-color, var(--neon-color))';

  return (
    <div
      className={`flex flex-col absolute top-0 right-0 h-full z-10 overflow-y-auto ${isClosing ? 'animate-[slideOut_0.15s_ease-in_forwards]' : 'animate-[slideIn_0.3s_ease-out]'}`}
      style={{
        width: '22.5rem',
        background: 'var(--bg-100)',
        borderLeft: '1px solid var(--border-200)',
      }}
    >
      <ScrollArea className="h-full">
        {/* Header */}
        <div className="flex items-center justify-between px-2 py-2 pl-4">
          <h3 className="flex items-center gap-2 min-w-0 text-lg font-medium" style={{ lineHeight: 1.125 }}>
            <Icon size={20} style={{ color, flexShrink: 0 }} />
            <span style={{ color }}>{entry.title}</span>
          </h3>
          <button
            className="flex items-center justify-center shrink-0 w-10 h-10 rounded-md border-none cursor-pointer transition-colors"
            style={{ color: 'var(--text-400)', background: 'none' }}
            onClick={onClose}
            aria-label="Close panel"
            onMouseEnter={(e) => { e.currentTarget.style.color = 'var(--text-100)'; e.currentTarget.style.background = 'var(--bg-200)'; }}
            onMouseLeave={(e) => { e.currentTarget.style.color = 'var(--text-400)'; e.currentTarget.style.background = 'none'; }}
          >
            <X size={20} />
          </button>
        </div>
        ...
        {/* Raw SQL */}
        <pre
          className="px-4 py-3 m-0 whitespace-pre-wrap break-words border-t"
          style={{
        ...
        {/* Encyclopedia */}
        <section className="px-4 pt-4 pb-5 border-t" style={{ borderColor: 'var(--border-200)' }}>
          <h4 className="text-sm font-normal lowercase mb-4" style={{ color: 'var(--text-400)' }}>Definition</h4>
          <p className="text-base font-light leading-relaxed mb-4" style={{ color: 'var(--text-100)' }}>{entry.definition}</p>
          <a
            href={entry.docsUrl}
            target="_blank"
        ...
        <section className="px-4 pt-4 pb-5 border-t" style={{ borderColor: 'var(--border-200)' }}>
          <h4 className="text-sm font-normal lowercase mb-4" style={{ color: 'var(--text-400)' }}>In this query</h4>
          <p className="text-base font-light leading-relaxed mb-4" style={{ color: 'var(--text-100)' }}>{renderWithCode(context)}</p>
        </section>
      </ScrollArea>
    </div>
  );
