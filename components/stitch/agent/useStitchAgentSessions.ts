...
import type { StitchProject } from '../../types';

export function useStitchAgentSessions({
  project,
  activeBoardIdx,
  isStreaming,
  messages,
  ...
  useEffect(() => {
    ...
    const loadSessions = async () => {
      try {
        const resp = await fetch(`/api/stitch-agent/sessions/${project.id}?boardIdx=${activeBoardIdx}`);
        if (!resp.ok) return;
        ...
        if (loadedSessions.length > 0) {
          ...
          try {
            const sessionResp = await fetch(`/api/stitch-agent/session/${latest.id}`);
            if (sessionResp.ok) {
        ...
        const createResp = await fetch('/api/stitch-agent/sessions', {
          method: 'POST',
  ...
  useEffect(() => {
    if (wasStreamingRef.current && !isStreaming && activeSessionId) {
      ...
      fetch(`/api/stitch-agent/sessions/${activeSessionId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleSwitchSession = useCallback(async (sessionId: string) => {
    ...
    onResetAgentState();
    try {
      const resp = await fetch(`/api/stitch-agent/session/${sessionId}`);
      if (resp.ok) {
        const data = await resp.json();
  ...
  const handleNewSession = useCallback(async () => {
    ...
    try {
      const resp = await fetch('/api/stitch-agent/sessions', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ projectId: project.id, boardIdx: activeBoardIdx }),
      });
  ...
  const handleDeleteSession = useCallback(async (sessionId: string) => {
    try {
      const resp = await fetch(`/api/stitch-agent/sessions/${sessionId}`, { method: 'DELETE' });
      if (resp.ok) {
        setSessions(prev => {
