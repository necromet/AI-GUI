...
import { Layers, Plus, Trash2, Globe } from 'lucide-react';
import { SkemaProject, SkemaProjectType, ModelConfig } from '../types';
...
const PROJECT_TYPES: { value: SkemaProjectType; label: string; desc: string; icon: React.ReactNode }[] = [
  { value: 'html', label: 'HTML Generator', desc: 'AI-powered interactive HTML editor', icon: <Globe size={28} /> },
];
...
const SkemaPanel: React.FC<SkemaPanelProps> = ({ theme = 'dark', onNotification, modelConfig, models, onProjectChange, onControlsChange, initialProjectId }) => {
  ...
  useEffect(() => {
    if (initialProjectId) {
      (async () => {
        try {
          const dbProject = await db.getSkemaProject(initialProjectId);
          console.log('[SkemaPanel] loaded project:', dbProject ? { id: dbProject.id, title: dbProject.title, boards_json_length: dbProject.boards_json?.length } : null);
          if (dbProject) {
            const project = skemaDBToProject(dbProject);
            console.log('[SkemaPanel] deserialized — boards:', project.boards.length, 'board0.generatedHtml length:', project.boards[0]?.generatedHtml?.length);
            setActiveProject(project);
            onProjectChange?.(project);
          }
        } catch (err) {
          console.error('Failed to load skema project:', err);
        } finally {
          setIsLoading(false);
        }
      })();
  ...
  }, [initialProjectId, loadProjects, onProjectChange]);
  ...
  const handleSaveProject = useCallback(async (updatedProject: SkemaProject) => {
    ...
    const dbPayload = skemaProjectToDB(projectToSave);
    console.log('[SkemaPanel] saving — boards:', dbPayload.boards.length, 'board0.generatedHtml length:', dbPayload.boards[0]?.generatedHtml?.length);
    await db.saveSkemaProject(dbPayload);
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs" style={{ color: 'var(--text-500)' }}>AI-powered interactive HTML editor</p>
          </div>
        </div>
        <Button
            ...
            {!selectedType ? (
              ...
              <>
                ...
                <div className="flex gap-2">
                  <Button
                    onClick={() => handleCreateProject(selectedType)}
                    className="rounded-xl"
                    style={{ backgroundColor: 'var(--neon-color)', color: '#000' }}
                  >
                    Create
      ...
      {isLoading ? (
        ...
        >
          <CardContent className="p-12 text-center">
            ...
            <p className="text-xs" style={{ color: 'var(--text-500)' }}>Create your first HTML project to get started</p>
          </CardContent>
        </Card>
      ) : (
              ...
              <div className="aspect-video relative overflow-hidden" style={{ backgroundColor: 'var(--bg-300)' }}>
                {project.boards[0]?.generatedHtml ? (
                  ...
                  <div className="w-full h-full flex items-center justify-center">
                    <Globe size={32} style={{ color: 'var(--text-500)' }} />
                  </div>
                )}
