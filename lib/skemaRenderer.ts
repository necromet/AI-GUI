...
function renderBackground(bg: SkemaBackground): string {
  switch (bg.type) {
    case 'solid':
...
function renderPatternOverlay(bg: Extract<SkemaBackground, { type: 'pattern' }>, w: number, h: number): string {
  const c = bg.color;
  switch (bg.pattern) {
    case 'dots': {
      let dots = '';
...
function renderElement(el: SkemaElement, theme: SkemaTheme): string {
...
function getLayoutStyles(layout: SkemaSlideLayout, theme: SkemaTheme): string {
...
export function renderSlide(spec: SkemaSlideSpec, theme: SkemaTheme, layout: SkemaLayout): string {
...
export function renderAllSlides(spec: SkemaDesignSpec, layout: SkemaLayout): string[] {
...
export function applySpecEdits(spec: SkemaDesignSpec, edits: { path: string; value: any }[]): SkemaDesignSpec {
